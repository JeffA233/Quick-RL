...
use crate::models::model_base::{CriticPPO, DiscreteActPPO};
...
impl PPOLearner {
    ...
    pub fn do_calc(
        ...
        for _epoch in 0..self.n_epochs {
            ...
            let batch_indexes =
                Tensor::randperm(self.total_buffer_size, (Kind::Int64, self.device));
            for batch_start_index in (0..self.total_buffer_size).step_by(self.optim_batch_size) {
                let buffer_indexes = batch_indexes.slice(
                    0,
                    batch_start_index,
                    batch_start_index + self.optim_batch_size as i64,
                    1,
                );
                ...
                let dist_entropy_float =
                    tch::no_grad(|| f32::try_from(&dist_entropy.detach()).unwrap());
                ...
                let value_loss = &vals
                    .mse_loss(&targ_vals.squeeze(), tch::Reduction::Mean)
                    .squeeze();
                ...
                let action_loss = -((&ratio * &advs)
                    .min_other(&(&clip_ratio * &advs))
                    .mean(Kind::Float));
